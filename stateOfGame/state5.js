...
// alert( -canvas.height / 2 + verticalPadding); - mislim da ovo moze da se korist
// za granicu kod 
        ...
        if (e.components.position_x !== undefined && e.components.position_y !== undefined) {
            ...
            for (triangle of entities) {
                ...
                if (triangle.components.x !== undefined && triangle.components.h !== undefined && triangle.components.random !== undefined) {
                    ...
                    // Dodavanje trougla u passedTrougao kad x ballona predje x trougla
                    if (bX >= triangleTop.x) {
...
scoringSystem = function (entities) {
    ...
    for (e of entities) {
        if (e.components.passedTriangles !== undefined) {
            const passedTriangles = e.components.passedTriangles.value;

            if(numberOfObstacles != 0){
                if(passedTriangles.length == numberOfObstacles){
                    e.components.heatingSound.value.pause();
                    e.components.gameStarted.value = false;
                    resetGame();
                    restartButton.textContent = "PLAY AGAIN"
                    restartButton.style.display = "block";
                    alert("Bravo,uspjesno ste zavrsili level");
                }
            }
           
            score.textContent = "Obstacles passed: " + passedTriangles.length;
        }
        newEntities.push(e);
    }
    return newEntities;
