...
entities.push(new Entity()
.addComponent(new Component("obstaclesNumber",0))
.addComponent(new Component("obstaclesPassed",0))
...
function generateTriangles(numberOfTriangles) {
    let newEntities = []; // Stvorite novu varijablu umjesto korištenja globalne entities
    ...
    for (let i = 0; i < numberOfTriangles; i++) {
        ...
        if (i == 0) {
            ...
            x = newEntities[i - 2].components.x.value + minimumGap + Math.floor(Math.random() * (maximumGap - minimumGap));
        }
    ...
    entities = entities.concat(newEntities); // Dodajte novu listu entiteta u globalne entitete
}
...
userInputSystem = (function inputSystem(){
    ...
    function getObstacleNumber(e){
        ...
        if(clickedButton == "level_1"){
            entities[1].components.obstaclesNumber.value = 50;
        }else if(clickedButton == "level_2"){
            entities[1].components.obstaclesNumber.value = 100;
        }

        level_1Button.style.display = "none";
        level_2Button.style.display = "none";
        
        generateTriangles(entities[1].components.obstaclesNumber.value);
    }
...
heatingSoundSystem = function(entities){  // Moze i u physicsSystem al ako ne budem imao vise sistema ostaviti
    let newEntities = [];
    for(i of entities){
        if(i.components.obstaclesNumber !== undefined && i.components.obstaclesNumber.value !== 0){
...
scoringSystem = function(entities){
    ...
    for(e of entities){
        if(e.components.gameStarted !== undefined){
            if(e.components.gameStarted == true){
